...
NSNumber * saveImage(NSString * fullPath, UIImage * image, NSString * format, float quality)
{
    NSData* data = nil;
    ...
    return [NSNumber numberWithUnsignedInteger:data.length];
}
                  ...
                  callback:(RCTResponseSenderBlock)callback)
{
    ...
    [_bridge.imageLoader loadImageWithURLRequest:[RCTConvert NSURLRequest:path] callback:^(NSError *error, UIImage *image) {
        ...
        NSNumber * fileSize = saveImage(fullPath, scaledImage, format, quality);
        if (!fileSize) {
        ...
        callback(@[[NSNull null], @{
            @"path": fullPath,
            @"width": [NSNumber numberWithInt:scaledImage.size.width],
            @"height": [NSNumber numberWithInt:scaledImage.size.height],
            @"mime": [format isEqualToString:@"JPEG"]?@"image/jpeg":@"image/png",
            @"size": fileSize,
        }]);
    }];
}
